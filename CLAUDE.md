...
- **Cardmarket is blocked by automation detection, not by hosting.** Measured 2026-07-26 against a real product page: plain `curl` → `403 Just a moment`; headless Chrome → `403 Attention Required`; **headed** Chrome with a fresh profile → `403 Just a moment` — all three from a residential IP. The old `cdp` mode worked only because it borrowed a long-lived Chrome profile that already held a `cf_clearance` cookie, and that cookie is bound to the IP + TLS fingerprint + User-Agent that solved the challenge, so it cannot be exported to a server. Moving to Azure therefore does **not** get Cardmarket back, and neither does adding a headless browser. Don't re-litigate this without re-running `node cloud/probe-cardmarket.js direct` first. Price aggregators (Scryfall `prices.eur`, MTGJSON) reach Azure fine but are 24h-synced trend/average figures, not live listings — rejected. Full write-up: `docs/azure-hosting.md`.
