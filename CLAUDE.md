...
# Serve the dashboard for manual testing in the user's OWN Chrome (the only way to
# exercise the real extension against live Cardmarket — see the Playwright gotcha below).
...
- **Don't try to verify the Cardmarket extension with Playwright — it can't work by design.** Measured 2026-07-26: Cloudflare serves a CDP-driven browser a *non-interactive* managed challenge that never completes (page stuck on "Performing security verification", **zero** buttons/checkboxes), so no human can click through it and warming the profile by hand is impossible. Automating the browser is exactly what's detected. Two adjacent traps: **Chrome 137+ removed `--load-extension`** on the stable channel (verified: `channel:"chrome"` → `serviceWorkers().length === 0`, bundled Chromium → `1`), so a "launch real Chrome with the extension" harness silently tests nothing; and clearance never transfers between browsers (bound to IP + TLS fingerprint + UA). Use `node scripts/try-extension.js --serve` and open the printed URL in the user's own everyday Chrome — that's the only real validation path. `test/cardmarket-client.browser.test.js` covers our own code against a stubbed bridge, which is the part that can actually regress.
