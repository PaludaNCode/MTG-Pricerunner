...
Data shape contract (produced by both `local/server.js` and `cloud/fetch-cardtrader.js`, consumed by `render.js`): `{ updatedAt, results: [{ site, group, variant, code, productUrl, error?, offers: [{ price, priceStr, foil, condition, qty, seller, shipsToMe, id?, firstSeenAt? }] }] }`. Offers are merged per `group` and sorted by price client-side. `code` is the official Scryfall set code from `config.json`, shown instead of `variant` on phones; `render.js` falls back to `variant` when it's missing (data.json predating the field). `id`/`firstSeenAt` are cloud-only for now: `id` is the CardTrader marketplace listing id (unique, stable per listing), and `fetch-cardtrader.js` stamps `firstSeenAt` by diffing ids against the previous `data`-branch snapshot. `render.js` highlights offers first seen within `newWindowMs` (default 24h) — reload-proof and shared across devices; offers without the stamp fall back to the old session-only diff.
